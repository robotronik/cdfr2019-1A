...
{
    volatile int Encval_droite=-encoder_droite->steps;
    volatile int Encval_gauche=encoder_gauche->steps;
...
void init(Encoder* encoder_droite,Encoder* encoder_gauche,int nbpoint,int epsilon,Point* cap,int* ptheta,unsigned int* cnt_droite,unsigned int* cnt_gauche,
  unsigned int* pdistanceparcourue_droite,unsigned int* pdistanceparcourue_gauche,int* px,int* py,int* moy_prev,int* pconsigne_gauche,int* pconsigne_droite){
    ...
    *pconsigne_droite=255;
    ...
    cap->x=65;
    cap->y=0;
    init_encoder(encoder_droite , &htim2);
    /*
    int i;
    for(i=0;i<nbpoint;i++){
...
void navigation(Encoder* encoder_gauche,Encoder* encoder_droite,int nbpoint,int epsilon,Point* cap,int* ptheta,unsigned int* cnt_droite,unsigned int* cnt_gauche,unsigned int* pdistanceparcourue_droite,
  unsigned int* pdistanceparcourue_gauche,int* px,int* py,int* moy_prev,int* pconsigne_gauche,int* pconsigne_droite){
    ...
    for(i=0;i<nbpoint;i++){
      ...
      while(epsilon>erreur_x || epsilon>erreur_y){
        ...
        update_encoder(encoder_gauche);
        update_encoder(encoder_droite);
        theta(ptheta,encoder_droite->steps,encoder_gauche->steps,32);
        mise_a_jour_distance( encoder_droite,encoder_gauche,pdistanceparcourue_droite,pdistanceparcourue_gauche);
